...
const IScroll = require('../../utils/iscroll-zoom')
...
var controller = (function ($, window, document) {
	...
	function photoClip(container, option) {
		...
		function resetScroll() {
			...
			var posX = (clipWidth - imgWidth * myScroll.options.zoomStart) * .5,//修改
		...
		function compressImg(sourceImgObj, quality, angleOffset, outputFormat){
			...
			var newImageData = cvs.toDataURL(mimeType, quality || .5);
			return newImageData;
		}
	...
	var data = {userId:userData.userId,
				headImage:'',
				nikeName:'',
				sex:'',
				age:'',
	...
	$(".address-modal>ul").css({"height":$(window).height()+'px'})

	var id = "#province"

	//---------------------
	$("#provinceCity").on('click',function(){
		$(".address-modal").show();
		...
		for(var i =0;i<province.length;i++){
			...
			_html+="<li onclick=\'cityfn("+code+","+i+")\'>"+province[i]['name']+"</li>";
		}
		

		$("#province").html(_html).attr("class","slideInRight");
	});

	var provinceCode = 110000,provinceCodeI,addressProvince,addressCity;
	window.cityfn = function (c,b){
			id = "#city"
			var _html='',a;
			var list = city[c],code;

			for(var i =0;i<list.length;i++){
				code = list[i]['code']
				_html+="<li onclick=\'selectAddress("+code+","+i+")\'>"+list[i]['name']+"</li>"
			}
			
			$("#city").html(_html).attr("class","slideInRight");
			$("#province").attr("class","slideOutLeft");
			provinceCode = c;
			provinceCodeI = b;
		}


	//点击城市是就是选择完成时候，关闭层，回复初始值

	window.selectAddress = function (code,i){	
			addressProvince = province[provinceCodeI]["name"]

			addressCity = city[provinceCode][i]["name"] ;
			$("#provinceCity").html(`${addressProvince} ${addressCity}`)
			$(".address-modal").hide();
			id = "#province";
			$(id).attr("class","hide");
			$("#city").attr("class","hide");
			data.province =addressProvince
			data.city = addressCity
		}
	...
	$("#returnPage").on("click",function(){
		if(id=="#province"){
			...
			$(id).attr("class","slideOutRight");
			$("#province").attr("class","slideInLeft ")
	...
	// ---------file---------------
		...
		function modalClipArea(){
			$("#clipModal").show();
		}
		//---------------------选择完图片时候-------------
		function saveFlie(file){
			var filenew = ''+file;
			let dataf = {headImage:filenew}
				dataf = JSON.stringify(dataf)
				
			$.ajax({
						type: "post",
						url: baseUrl + '/appuser/upload',
						data: dataf,
						...
						success: function (json) {
									if (json.status == "0") {
									 	data.headImage = json.data
										$("#imgClip").attr("src",data.headImage);
									} else {
										noticeSetTimeoutPublick("提交失败！")
		...
		var nickname ,sex = ""//1 men,2women;
		$("#nameInput").on("input",function(){
			var value = $(this).val();
			nickname = value.replace(/<\/?[^>]*>/gim,"").replace(/\s/g,"");
			if (!nickname) {
				nickname = ""
			}
			$("#name").html(nickname)
	...
	//提交
		$("#submitBtn").on("click",function(){
			data.nikeName = $("#name").html();
			data.age = $('#oldInput').val();
			data.userId = userData.userId;
			$.ajax({
						type: "post",
						url: baseUrl + '/appuser/update',
						data: data,
						beforeSend: function (XMLHttpRequest) {
									loadShow()
								},
						success: function (json) {
									...
									localStorage.setItem('baseDate',setDate)
										noticeSetTimeoutPublick("提交成功！")
		...
		function init() {
			...
			var province = baseDate.province?baseDate.province:'省／',
				city = baseDate.city?baseDate.city:'市／区',
				sex = baseDate.sex?baseDate.sex:'女',
				nikeName = baseDate.nikeName?baseDate.nikeName:'昵称';
			data = {userId:baseDate.userId,
					headImage:baseDate.headImage,
					nikeName:baseDate.nikeName,
					sex:baseDate.sex,
					age:baseDate.age,
					province:baseDate.province,
					city:baseDate.city
				};//上传给后台时
				
				$("#name").html(nikeName);
				$('#sex').html(sex);
				$("#imgClip").html(data.headImage);
				$('#oldInput').val(data.age);
				$("#provinceCity").html(`${province} ${city}`)
				$("#imgClip").attr('src',data.headImage)
